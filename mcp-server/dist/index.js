...
const TOOLS = [
    ...
    {
        name: "get_user_stats",
        description: "Returns watch history, lists, and profile data for a user identified by email. Useful for debugging user-reported issues.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "seed_titles",
        description: "Triggers the tmdb-cache discover action to seed N pages of popular titles. Each page = ~20 titles. Service-role auth used.",
        inputSchema: {
            type: "object",
            properties: {
                media_type: {
    ...
    },
    {
        ...
        description: "Triggers the generate-embedding function to backfill embeddings for titles without one. Service-role auth used.",
        inputSchema: {
            type: "object",
            properties: {
                limit: {
    ...
    {
        name: "title_lookup",
        description: "Returns full details about a specific title: metadata, embedding status, watch history count, list memberships.",
        inputSchema: {
            type: "object",
            properties: {
                title_id: {
    ...
    },
    {
        ...
        description: "Returns the last N watch_history entries across all users. Useful for debugging real-time activity issues.",
        inputSchema: {
            type: "object",
            properties: {
                limit: {
    ...
    {
        name: "search_catalog",
        description: "Quick text search of the titles table (no embedding required). Returns up to 20 matches.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Tests an edge function with a GET request and returns the response. Useful for quick endpoint verification.",
        inputSchema: {
            type: "object",
            properties: {
                function_name: {
    ...
    },
    {
        ...
        description: "Analyzes database query performance. Returns slow queries, missing indexes, and table sizes. Helps identify performance bottlenecks.",
        inputSchema: {
            type: "object",
            properties: {
                threshold_ms: {
    ...
    },
    {
        ...
        description: "Checks health of key tables: row counts, last updated, index presence, bloat estimation.",
        inputSchema: {
            type: "object",
            properties: {
                table_name: {
    ...
    {
        name: "find_errors",
        description: "Searches watch_history and profiles for anomalies: NULL values, invalid status values, orphaned records. Helps identify data quality issues.",
        inputSchema: {
            type: "object",
            properties: {
                table_name: {
    ...
    },
    {
        ...
        description: "Returns embedding backfill status: total titles, embedded count, missing count, and sampling of unembedded titles.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    },
    {
        ...
        description: "Returns information about RPC functions and their typical execution times. Helps identify slow browse_titles, semantic-search, etc.",
        inputSchema: {
            type: "object",
            properties: {
                limit: {
    ...
    },
    {
        ...
        description: "Query recent browser console logs captured from real users. Filter by level (error/warn/log/info), page, or time window. Returns message, page, session, timestamp.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "get_error_logs",
        description: "Query captured frontend errors (unhandled exceptions and promise rejections). Includes stack traces and page context. Most useful for debugging production crashes.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Query captured fetch request timings from real users. Find slow requests, failed calls (status 0 or 5xx), and Supabase RPC latency.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Query Core Web Vitals (LCP, FCP, CLS, TTFB) captured from real user sessions. Group by page to find slow routes.",
        inputSchema: {
            type: "object",
            properties: {
...
async function handleGetUserStats(email) {
    // Find user by email
...
async function handleSeedTitles(media_type, pages) {
...
async function handleBackfillEmbeddings(limit = 50) {
...
async function handleTitleLookup(title_id) {
    ...
    if (!title.data)
        return { error: `Title ${title_id} not found` };
    return {
        metadata: title.data,
...
async function handleRecentActivity(limit = 20) {
...
async function handleSearchCatalog(query, media_type) {
    let q = supabase
        .from("titles")
...
async function handleEdgeFunctionTest(function_name, query_string) {
...
async function handleCheckTableHealth(table_name) {
    ...
    if (table_name === "watch_history")
        updated_at_col = "watched_at";
    if (table_name === "list_items")
        updated_at_col = "added_at";
...
async function handleFindErrors(table_name) {
...
async function handleGetSlowRpcCalls(limit = 10) {
    const rpcs = [
...
async function queryRecent(opts) {
