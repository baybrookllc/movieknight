-- ═══════════════════════════════════════════════════════════════
-- KEYWORD SEARCH RPC — server-side full-text search fallback
-- Replaces client-side filtering in semantic-search edge function.
-- Uses PostgreSQL GIN + tsvector for efficient text matching.
-- ═══════════════════════════════════════════════════════════════

-- ── 1. GIN index on tsvector(title, overview) ────────────────────
-- Needed for fast plainto_tsquery matches on large title catalogs.
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Server-side keyword search ranked by ts_rank.
-- Returns same shape as semantic search for drop-in compatibility.
